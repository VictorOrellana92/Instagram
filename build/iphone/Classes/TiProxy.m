...
//Rare exceptions to indicate a bug in the _instagram code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_instagram = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_instagram]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.instagram.module"
